...
func main() {
	...
	divide, _ := calculator.Divide(20, 4)
	// TODO: Используй функцию FormatResult из пакета formatter
	// для вывода результатов в формате: "Результат: X"

	fmt.Println(formatter.FormatResult("Add", add))
	fmt.Println(formatter.FormatResult("subtract", subtract))
	fmt.Println(formatter.FormatResult("multiply", multiply))
	fmt.Println(formatter.FormatResult("divide", divide))
